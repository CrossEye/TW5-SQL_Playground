const parse = (s, now = Date.now()) => s.split('\n')
  .filter(Boolean)
  .filter(s => s.startsWith('INSERT INTO'))
  .map(parseLine)
  .map(({title, tags, ...rest}, i) => ({
    title,
    tags,
    ...(tags == "Customer" 
      ? {caption: `${rest['first-name']} ${rest['last-name']}`} 
      : tags == "Supplier"
      ? {caption: rest['company-name']}
      : tags == "Product"
      ? {caption: rest['product-name']}
      : {caption: title}
    ),
    created: new Date(now - i).toISOString().replace(/\D/g, ''),
    modified: new Date(now - i).toISOString().replace(/\D/g, ''),
    ...rest
  }))
  .map(updateFields)

const parseLine = (line) => extract(
  line.match(/INSERT INTO \[(\w+)\] \(([^\)]+)\)VALUES\((.+)\)/).slice(1)
)
const zipObj = (xs, ys) => Object.fromEntries(xs.map((_, i) => [xs[i], ys[i]] ))
const extract = ([t, ks, vs]) => makeTid([t, zipObj(
  ks.split(',').map(s => camel2kebab(s.slice(1, -1))), 
  parseValues(vs)
)])
const makeTid = ([tag, cols]) => ({title:`${tag}/${cols.id}`, tags: tag, ...cols})
const camel2kebab = (s) => s.replace(/[A-Z]+(?![a-z])|[A-Z]/g, ($, x) => (x ? "-" : "") + $.toLowerCase())
const convertDate = (str, [_, M, d, y, h, m, s, f, a] = str.match(/([a-z]+)\s+(\d+)\s+(\d+)\s+(\d+)\:(\d+):(\d+).(\d+)([a-z]+)/i)) => 
  new Date(Date.parse(`${d} ${M} ${y} ${a == "PM" ? Number(h) + 12 : h}:${m}:${s}.${f} GMT`)).toISOString().replace(/\D/g, '')

const updateFields = (rules => (o) => Object.fromEntries(Object.entries(o) .map(
  ([k, v]) => [k, (rules[k] || ((v, o) => v))(v, o)]
)))({
  'is-discontinued': (v) => v == '0' ? 'no' : 'yes',
  'order-date': (v) => convertDate(v)
})



// import stateMachine, {DFLT, EOL, STATE} from 'stateMachine'
// Above is expected usage.  Inlined instead here:

const stateMachine = Object.assign(
  (initState, extract, rules) => ([...chars]) => 
    extract([...chars, stateMachine.EOL].reduce((state, c) => ({
      ...state,
      [STATE]: (rules[state[STATE]][c] || rules[state[STATE]][DFLT]) [0],
      ...((rules[state[STATE]][c] || rules[state[STATE]][DFLT])[1] || ((c, s) => ({})))(c, state)
    }), initState)), {
  DFLT: Symbol(),
  EOL: Symbol(),
  STATE: Symbol(),
})  
const {DFLT, EOL, STATE} = stateMachine

const concatCurr = (c, o) => ({curr: o.curr.concat(c)})
const addItem = (c, o) => ({all: o.all.concat(o.curr), curr: ''})
const errBadQuotes = (c, o) => ({curr: '', all: [], err: 'Unbalanced quotations'})
const complete = (c, o) => ({all: o.curr.length ? o.all.concat(o.curr) : o.all, curr: ''})

const parseValues = stateMachine(
  {[STATE]: 'WAITING', all: [], curr: ''},
  ({err, all}) => err ? {err} : all, 
  {
    WAITING: {"'": ['QUOTED'], ",": ['WAITING'], [EOL]: ['WAITING', complete], [DFLT]: ['PLAIN', concatCurr],},
    ONEQUOTE: {",": ['WAITING', addItem], "'": ['QUOTED', concatCurr], [EOL]: ['WAITING', addItem], [DFLT]: ['ERROR', errBadQuotes]},
    QUOTED: {"'": ['ONEQUOTE'], [EOL]: ['ERROR', errBadQuotes], [DFLT]: ['QUOTED', concatCurr],},
    PLAIN: {",": ['WAITING', addItem], "'": ['ERROR', errBadQuotes], [EOL]: ['WAITING', addItem], [DFLT]: ['PLAIN', concatCurr],},
    ERROR: {[DFLT]: ['ERROR', (c, o) => ({curr: '', all: []})],}
  }
)

console.clear()

const main = () => console.log(JSON.stringify(parse(lines), null, 4))


// Data from https://www.dofactory.com/sql/sample-database

const lines = `SET IDENTITY_INSERT Customer ON
